...
export default function OfferingsResultsPage() {
  ...
  const categoryOptions = useMemo(() => {
    return Array.from(
      new Set(
        allData
          .map((item) => normalizeValue(item.category))
          .filter(Boolean)
      )
    ).sort();
  }, [allData]);
  ...
  const cityOptions = useMemo(() => {
    return Array.from(
      new Set(
        allData
          .map((item) => normalizeValue(item.city))
          .filter(Boolean)
      )
    ).sort();
  }, [allData]);
  ...
  const experienceOptions = useMemo(() => {
    return Array.from(
      new Set(
        allData
          .map((item) => normalizeValue(item.experience))
          .filter(Boolean)
      )
    ).sort();
  }, [allData]);
    ...
    <div className="relative h-screen w-full overflow-hidden bg-black text-white sync-page-with-bottom-nav">
      ...
      {isChatOpen && !loading && (
        <OfferingsChatbot
          ...
          onResultSelect={(items: OfferingItem[]) => {
            const safeItems = Array.isArray(items) ? items : [];
            setVisibleData(safeItems);
            setSelected(null);
          }}
        />
      )}
